...
$("globalStat").textContent=`${totalWords} mục từ • ${textCount} bài khóa • v15`;
...
let yuhuaChineseVoice = null;
let yuhuaVoicesLoaded = false;
...
function chooseChineseFemaleVoice(){
  ...
  for(const wanted of preferredNames){
    const found = voices.find(v =>
      String(v.name||"").toLowerCase().includes(wanted.toLowerCase())
    );
    if(found) return found;
  }

  const femaleHints = [
    "xiaoxiao","ting","mei","jia","sin","female","woman","女","普通话"
  ];

  const zhCN = voices.filter(v =>
    /^zh[-_]CN$/i.test(v.lang || "") ||
    /^cmn[-_]Hans[-_]CN$/i.test(v.lang || "") ||
    String(v.lang||"").toLowerCase().startsWith("zh-cn")
  );

  const hinted = zhCN.find(v =>
...
function refreshChineseVoice(){
  yuhuaChineseVoice = chooseChineseFemaleVoice();
  yuhuaVoicesLoaded = true;
}

if("speechSynthesis" in window){
  refreshChineseVoice();
  speechSynthesis.onvoiceschanged = refreshChineseVoice;
}
...
function speak(text){
  ...
  if(!yuhuaVoicesLoaded || !yuhuaChineseVoice){
    refreshChineseVoice();
  }
  ...
  const speakNext = () => {
    ...
    if(yuhuaChineseVoice){
      u.voice = yuhuaChineseVoice;
    }
...
window.speak = speak;
window.refreshChineseVoice = refreshChineseVoice;
