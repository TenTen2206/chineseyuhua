...
const HSK_LEVELS = [
  {level:1, title:"HSK 1", status:"coming", desc:""},
  {level:2, title:"HSK 2", status:"ready", desc:"15 bài theo New HSK Course 2."},
  {level:3, title:"HSK 3", status:"ready", desc:""},
  {level:4, title:"HSK 4", status:"coming", desc:""},
  {level:5, title:"HSK 5", status:"coming", desc:""},
  {level:6, title:"HSK 6", status:"coming", desc:""}
];
let selectedHSKLevel = 3;

function currentLevelData(){
  if(selectedHSKLevel===2) return HSK2_DATA;
  if(selectedHSKLevel===3) return HSK3_DATA;
  return [];
}
function isHSK2(){ return selectedHSKLevel===2; }

const totalWords=HSK3_DATA.reduce((s,l)=>s+(l.words?.length||0),0);
const textCount=HSK3_DATA.reduce((s,l)=>s+(l.texts?.length||0),0);
$("globalStat").textContent=`HSK 2: 15 bài • HSK 3: ${textCount} bài khóa • v25`;
...
function renderLessons(){
  ...
  if(selectedHSKLevel!==2 && selectedHSKLevel!==3){
    const lvl=HSK_LEVELS.find(x=>x.level===selectedHSKLevel);
    container.innerHTML=`
      <div class="coming-level simple-placeholder">
  ...
  const levelLabel=`HSK ${selectedHSKLevel}`;
  container.innerHTML=`<h2 class="lesson-heading">Các bài ${levelLabel}</h2><div id="lessonGrid" class="lesson-grid"></div>`;
  const grid=$("lessonGrid");
  grid.innerHTML=data.map(l=>{
    if(selectedHSKLevel===2){
      const textCount=(l.texts||[]).length;
      const wordCount=(l.words||[]).length;
      return `<button class="lesson-card" onclick="openLesson(${l.n})">
        <div class="lesson-num">BÀI ${l.n}</div>
        <h3>${escapeHtml(l.title)}</h3>
        <div class="grammar-mini">${(l.grammar||[]).map(g=>`<span>${escapeHtml(g)}</span>`).join("")}</div>
        <div class="muted" style="margin-top:10px">Trang ${l.page} • ${wordCount} từ${textCount?` • ${textCount} bài khóa`:""}</div>
      </button>`;
    }
  ...
  $("homePanel").innerHTML=`
    <div class="home-panel-card">
      <div class="panel-title-row">
        <div>
          <div class="kicker">Giáo viên</div>
          <h2>Thông tin giáo viên</h2>
        </div>
        <button class="ghost" onclick="closeHomePanel()">Đóng</button>
      </div>
      ...
      <div class="teacher-profile">
        ...
        <div class="teacher-details">
          <h3>YuHua Chinese</h3>
          <p>Giáo viên tiếng Trung</p>
          <div class="info-note">
            Bạn có thể chỉnh tên giáo viên, thông tin liên hệ, kinh nghiệm giảng dạy và lời giới thiệu trong file <b>app-v19.js</b>.
          </div>
        </div>
      </div>
    </div>`;
...
function renderHomeClass(){
  $("homePanel").innerHTML=`
    <div class="home-panel-card">
      <div class="panel-title-row">
        <div>
          <div class="kicker">Lớp học</div>
      ...
      <div class="class-info-grid">
        <div class="class-info-item">
          <span>📘</span>
          <div><b>Chương trình học</b><p>HSK 1 – HSK 6</p></div>
        </div>
        <div class="class-info-item">
          <span>🗓️</span>
          <div><b>Lịch học</b><p>Thông tin lịch học có thể bổ sung tại đây.</p></div>
        </div>
        <div class="class-info-item">
          <span>🎯</span>
          <div><b>Mục tiêu</b><p>Học từ vựng, bài khóa và luyện tập theo từng trình độ.</p></div>
        </div>
      </div>
    </div>`;
}
...
function renderHomeReview(){
  $("homePanel").innerHTML=`
    <div class="home-panel-card">
      <div class="panel-title-row">
        <div>
          <div class="kicker">Ôn tập bài học</div>
          <h2>Chọn trình độ HSK</h2>
        </div>
        <button class="ghost" onclick="closeHomePanel()">Đóng</button>
      </div>

      <div id="levelGrid" class="level-grid review-level-grid"></div>

      <div id="selectedLevelPanel" class="selected-level-panel">
        <div class="level-panel-head">
          <div>
            <div class="kicker" id="selectedLevelKicker">Trình độ đang chọn</div>
            <h2 id="selectedLevelTitle">HSK 3</h2>
            <p id="selectedLevelDesc" class="muted"></p>
          </div>
          <span id="selectedLevelStatus" class="badge complete" style="display:none"></span>
        </div>

        <div id="hsk3ProgressWrap" class="home-progress">
          <div>
            <div class="kicker">Tiến độ HSK 3 của bạn</div>
            <h3 id="overallProgressText">0% hoàn thành</h3>
          </div>
          <div class="progress-track"><div id="overallProgressBar" class="progress-bar"></div></div>
          <button id="resetProgressBtn" class="ghost">Đặt lại tiến độ</button>
        </div>
...
window.openHomePanel=type=>{
  const panel=$("homePanel");
  panel.classList.remove("hidden");

  if(type==="teacher") renderHomeTeacher();
  else if(type==="class") renderHomeClass();
  else renderHomeReview();

  setTimeout(()=>panel.scrollIntoView({behavior:"smooth",block:"start"}),50);
};

window.closeHomePanel=()=>{
  const panel=$("homePanel");
...
function renderLevelGrid(){
  ...
  grid.innerHTML=HSK_LEVELS.map(l=>`
    <button class="level-card ${selectedHSKLevel===l.level?"active":""}"
...
window.selectHSKLevel=level=>{
  ...
  $("hsk3ProgressWrap").style.display = level===3 ? "" : "none";
...
window.openLesson=n=>{
  currentLesson=currentLevelData().find(x=>x.n===n);
  if(!currentLesson)return;
  reviewIndex=0;translationIndex=0;
  $("home").classList.remove("active");$("lesson").classList.add("active");
  $("backBtn").textContent=`← HSK ${selectedHSKLevel} / Các bài`;
  $("lessonNo").textContent=`BÀI ${currentLesson.n} • HSK ${selectedHSKLevel}`;
  $("lessonTitle").textContent=currentLesson.title;

  if(isHSK2()){
    $("lessonStatus").innerHTML=`
      <div class="hsk2-source-meta">
        <span class="badge complete">New HSK Course 2</span>
        <span class="muted">Trang bắt đầu: ${currentLesson.page}</span>
        ${currentLesson.partial?'<span class="badge partial">Nội dung PDF kết thúc trong bài này</span>':""}
      </div>`;
    configureHsk2Modules();
    $("lessonBreadcrumb").textContent=`HSK 2 / Bài ${currentLesson.n}`;
    showModule("grammar");
  }else{
    $("lessonStatus").innerHTML=`
...
$("backBtn").onclick=()=>{
  ...
  renderHomeReview();
  selectHSKLevel(selectedHSKLevel);
  updateProgressUI();
  window.scrollTo({top:0,behavior:"smooth"});
};
...
function configureHsk2Modules(){
  document.querySelectorAll(".learning-step").forEach(s=>s.style.display="");
  setModuleVisible("grammar",true);
  setModuleVisible("vocab",(currentLesson.words||[]).length>0);
  for(let i=1;i<=4;i++) setModuleVisible(`text${i}`,(currentLesson.texts||[]).length>=i);
  setModuleVisible("review",(currentLesson.words||[]).length>0);
  setModuleVisible("translate",false);
  setModuleVisible("fill",(currentLesson.texts||[]).some(t=>(t.targets||[]).length>0));
  setModuleVisible("quiz",(currentLesson.words||[]).length>=4);
}
function configureHsk3Modules(){
  ["grammar","vocab","text1","text2","text3","text4","review","translate","fill","quiz"].forEach(m=>setModuleVisible(m,true));
  setModuleVisible("grammar",false);
}
  ...
  $("moduleContent").innerHTML=`
    <div class="text-head">
      <div>
        ...
        <div class="muted">Bài ${currentLesson.n} • HSK 2</div>
      </div>
    </div>
    <div class="grammar-list">
      ${grammar.map((g,i)=>`
        <div class="grammar-item">
          <div class="grammar-num">${i+1}</div>
          <div><b>${escapeHtml(g)}</b></div>
        </div>`).join("")}
    </div>
    <div class="source-note-compact">
      Nội dung tên bài và điểm ngữ pháp được nhập theo mục lục của giáo trình đính kèm.
    </div>`;
}
function showModule(mod){
  if(!currentLesson)return;
  if(mod==="grammar")return renderGrammar();
...
function renderVocab(){
  ...
  $("moduleContent").innerHTML=`
    <div class="text-head">
      <div><h3>Từ vựng</h3><div class="muted">${words.length} mục từ • Hán tự + pinyin + nghĩa tiếng Việt + ví dụ + phát âm</div></div>
      ${isHSK2()?"":completionButton("vocab")}
    </div>
    <div class="word-list">
      ${words.map((w,i)=>{
  ...
  $("moduleContent").innerHTML=`
    <div class="text-head">
      ...
      <div class="text-actions">
        ...
        ${isHSK2()?"":completionButton(`text${id}`)}
      </div>
    </div>
...
window.markRemembered=()=>{
  ...
  if(!isHSK2() && rememberedSet.size===makeReviewCards().length)setCompleted(currentLesson.n,"review",true);
  nextReview();
};
window.markNotRemembered=()=>{
  const h=reviewCards[reviewIndex].hanzi;
  notRememberedSet.add(h);rememberedSet.delete(h);saveReviewState();
  if(!isHSK2()) setCompleted(currentLesson.n,"review",false);
  nextReview();
};
...
window.checkFill=id=>{
  ...
  if(!isHSK2()) setCompleted(currentLesson.n,"fill",true);
};
...
function makeQuiz(){
  ...
  const questions=[];
  const selected=sample(words,Math.min(words.length,8));

  selected.slice(0,4).forEach(w=>questions.push({
    type:"meaning",
  ...
  selected.slice(4,8).forEach(w=>questions.push({
    type:"pinyin",
  ...
  sample(targetPool,Math.max(0,8-questions.length)).forEach(item=>{
  ...
  return sample(questions,Math.min(8,questions.length));
}
function renderQuiz(){
  ...
  $("moduleContent").innerHTML=`
    <div class="exercise-wrap">
      ...
      <p class="exercise-sub">${quizState.questions.length} câu ôn tập: nghĩa từ vựng, pinyin và điền từ theo bài khóa (nếu có).</p>
      <div class="quiz-list">
...
window.submitQuiz=()=>{
  ...
  if(!isHSK2()) setCompleted(currentLesson.n,"quiz",true);
};
